...
public class PPPathBuilder {
    ...
    public static final int CURVATURE_SAMPLE_SIZE = 2; // TODO tune;
    public static final int TANGENT_HEADING_SAMPLE_SIZE = 1; // TODO tune
    public static final double INJECT_SPACING_INCHES = 1.0; // TODO tune

    /**
     * Constructs a smooth curve using a few vertices. The heading will set as tangent to the curve.
     */
    public static PurePursuitPath buildPathFromVertices(double[] xVertices, double[] yVertices, double maxPower, boolean stopAtEnd) {
        ...
        double b = 0.144;
//        smooth(xVals, yVals, 1 - b, b, 1); // TODO fix parameters
    ...
    public static double[] computeOptimalPowers(double[] xVals, double[] yVals, double maxPower, boolean stopAtEnd) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (power > maxPower) {
                power = maxPower;
            } else if (power < PathFollowModule.MIN_POWER) {
                power = PathFollowModule.MIN_POWER;
            }
            powers[i] = power;
        }
    ...
    public static void limitAcceleration(double[] powerVals, double[] distAlongPaths) {
        ...
        for (int i = 0; i < n; i++) {
            if (powerVals[i] < PathFollowModule.MIN_POWER) {
                powerVals[i] = PathFollowModule.MIN_POWER;
            }
        }
        for (int i = 1; i < n; i++) {
     ...
     */
    public static void smooth(double[] xVals, double[] yVals, double weight_data, double weight_smooth, double tolerance) {
        int n = xVals.length;
        if (yVals.length != n) {
            throw new RuntimeException("Array lengths do no match");
        }
        double[][] newPath = new double[n][2];
        for (int i = 0; i < n; i++) {
            newPath[i][0] = xVals[i];
            newPath[i][1] = yVals[i];
        }
        ...
        while (change >= tolerance) {
            ...
            for (int i = 1; i < newPath.length - 1; i++)
                for (int j = 0; j < newPath[i].length; j++) {
                    ...
                    newPath[i][j] += weight_data * (newPath[i][j] - newPath[i][j]) + weight_smooth * (newPath[i - 1][j] + newPath[i + 1][j] - (2.0 * newPath[i][j]));
